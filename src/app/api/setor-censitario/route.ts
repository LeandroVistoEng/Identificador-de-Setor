...
export async function POST(request: NextRequest) {
  try {
    const body = await request.json() as SetorCensitarioRequest

    if (!body.tipo || (body.tipo === 'endereco' && !body.endereco) || 
        (body.tipo === 'coordenadas' && (!body.latitude || !body.longitude))) {
    ...
    if (body.tipo === 'endereco') {
      ...
      const geocodResult = await geocodificarEndereco(body.endereco!)
      
      if (!geocodResult) {
        return NextResponse.json(
          { error: 'Endereço não encontrado ou fora do Estado do Rio de Janeiro' },
          { status: 404 }
        )
      }
      ...
      if (resultado) {
        resultado.enderecoPesquisado = body.endereco!
      }
      ...
      if (latitude! < -23 || latitude! > -20 || longitude! < -45 || longitude! > -40) {
        return NextResponse.json(
          { error: 'Coordenadas fora do Estado do Rio de Janeiro' },
          { status: 400 }
        )
      }

      // Encontrar setor censitário pelas coordenadas
      resultado = encontrarSetorCensitario(latitude!, longitude!)
    }

    if (!resultado) {
      return NextResponse.json(
        { error: 'Setor censitário não encontrado para a localização informada' },
        { status: 404 }
      )
    }
    ...
    console.error('Erro na API de setor censitário:', error)
    return NextResponse.json(
